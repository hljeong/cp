#ifndef TMPL
#define TMPL
#include <bits/stdc++.h>
#define A first
#define B second
#define pb push_back
#define endl '\n'
#define tT template <class T
#define tTU tT, class U
using namespace std;
tT> using vc = vector<T>;
tT, int N> using ar = array<T, N>;
using ll = long long; using ld = long double;
using vi = vc<int>; using vll = vc<ll>;
using si = set<int>; using sll = set<ll>;
using pi = pair<int, int>; using pll = pair<ll, ll>;
using qi = queue<int>;
using mii = map<int, int>;
using str = string;

tT> ostream &operator<<(ostream &os, const vc<T> &a) {
  for (int i = 0, n = a.size(); i < n; i++) os << (i ? " " : "") << a[i];
  return os;
}

tT, size_t N> ostream &operator<<(ostream &os, const ar<T, N> &a) {
  for (int i = 0; i < N; i++) os << (i ? " " : "") << a[i];
  return os;
}

tTU> istream& operator>>(istream &i, pair<T, U> &p) { return i >> p.A >> p.B; }

tT> T rd() { T x; cin >> x; return x; }

tT> vc<T> rda(int n) {
  vc<T> v(n);
  for (int i = 0; i < n; i++) cin >> v[i];
  return v;
}

int ri() { return rd<int>(); }
ll rll() { return rd<ll>(); }
str rs() { return rd<str>(); }
vi ria(int n) { return rda<int>(n); }
vll rlla(int n) { return rda<ll>(n); }

tTU> constexpr decltype(declval<T>() + declval<U>()) max(T a, U b) { return a > b ? a : b; }
tTU> constexpr decltype(declval<T>() + declval<U>()) min(T a, U b) { return a < b ? a : b; }
tTU> constexpr bool ckmin(T &a, U b) { return b < a ? a = b, 1 : 0; }
tTU> constexpr bool ckmax(T &a, U b) { return a < b ? a = b, 1 : 0; }
tT> constexpr T nth_bit(T x, int n) { return (x >> n) & 1; }
tTU> T minst(T l, T r, U pred) {
  T ans = r + 1;
  while (l <= r) {
    T m = l + (r - l) / 2;
    if (pred(m)) r = (ans = m) - 1;
    else l = m + 1;
  }
  return ans;
}
tTU> T maxst(T l, T r, U pred) {
  T ans = l - 1;
  while (l <=+ r) {
    T m = l + (r - l) / 2;
    if (pred(m)) l = (ans = m) + 1;
    else r = m - 1;
  }
  return ans;
}

constexpr int inf = 1e9 + 7;
constexpr int mod = inf;
constexpr ll infll = 0x3f3f'3f3f'3f3f'3f3fll;

// ucla tryout #1 2025 virtual
// for each i find cost to make h[:i] nondecreasing and h[i:] nonincreasing
// use a stack to keep track of last uptick + index

void solve() {
  int n = ri(); vi h = ria(n);
  vc<pi> pstack(n + 1, {-inf, -1}), sstack(n + 1, {-inf, n});
  int psp = 1, ssp = 1;
  vll pcost(n + 1), scost(n + 1);
  for (int i = 0; i < n; i++) {
    pcost[i + 1] = pcost[i];
    while (h[i] < pstack[psp - 1].A) {
      psp--;
      pcost[i + 1] += ll(pstack[psp].A - h[i]) * (pstack[psp].B - pstack[psp - 1].B);
    }
    pstack[psp++] = {h[i], i};

    scost[n - i - 1] = scost[n - i];
    while (h[n - i - 1] < sstack[ssp - 1].A) {
      ssp--;
      scost[n - i - 1] += ll(sstack[ssp].A - h[n - i - 1]) * (sstack[ssp - 1].B - sstack[ssp].B);
    }
    sstack[ssp++] = {h[n - i - 1], n - i - 1};
  }
  ll ret = infll;
  for (int i = 0; i <= n; i++) {
    ckmin(ret, pcost[i] + scost[i]);
  }
  // cout << pcost << endl;
  // cout << scost << endl;
  cout << ret << endl;
}

int main() {
  cin.tie(0)->sync_with_stdio(0);

  int t = 1;
  // int t = ri();
  while (t--) solve();
}

#endif
