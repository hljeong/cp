...
void solve() {
  ...
  for (int i = 0; i < n; i++) {
    str code = rs(); int t = ri();
    if (!mp.count(code)) continue;
    while (edges.size() && -edges.top().A <= t) {
      auto [ecode, delta] = edges.top().B; edges.pop();
      active[ecode] += delta;
    }
    // cout << code << " " << t << endl;
    // cout << active << endl;
    ...
    auto &rng = nxt[ci];
    edges.push({-(t + rng.A), {ci + 1, 1}});
    edges.push({-(t + rng.B + 1), {ci + 1, -1}});
  }
  cout << "No" << endl;
}
