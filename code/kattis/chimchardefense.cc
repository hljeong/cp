...
// https://codeforces.com/gym/105833/attachments/download/31071/CS3233_2025_Final_Solution_Writeup.pdf
// dp from the back -- shouldve solved this in contest :(

void solve() {
  int n = ri();
  vi h = ria(n), d = ria(n), c = ria(n);
  vc<vll> dp(n + 1, vll(5001, -infll));
  dp[n][0] = 0;
  ll ans = 0;
  for (int i = n - 1; i >= 0; i--) {
    for (int dmg = 0; dmg <= 5000; dmg++) {
      ckmax(dp[i][dmg], dp[i + 1][dmg] + min(dmg, h[i]));
      int ndmg = min(5000, dmg + d[i]);
      ckmax(dp[i][ndmg], dp[i + 1][dmg] + min(ndmg, h[i]) - c[i]);
      ckmax(ans, dp[i][dmg]);
    }
  }
  cout << ans << endl;
