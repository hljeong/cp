...
// indeed joyless. horrible problem
// if abababa or ababab then bash wins immediately
// other cases depend on parity and whether the 2 ends have the same char
...
void solve() {
  ...
  if (st.size() == 2) {
    ...
    cout << (((s.size() & 1) ^ (s[0] == s[s.size() - 1])) ? "Chikapu" : "Bash") << endl;
  }
}
