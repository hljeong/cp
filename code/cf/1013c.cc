...
// split a into 2 halves such that product of the 2 ranges is minimized
// either split down the middle, or one is a contiguous range within the other
// basically try all cyclic shifts
...
void solve() {
  ...
  for (int i = 1; i < n; i++) {
    ckmin(ans, (a[2 * n - 1] - a[0]) * (a[i + n - 1] - a[i]));
  }
  cout << ans << endl;
}
