...
void solve() {
  ...
  for (int p = 0; p < m; p++)
    sort(party[p].begin(), party[p].end());
  ll ret = infll;
  for (int tot = max(1, party[0].size()); tot <= n; tot++) {
    ll cost = 0;
    // not 0-initializing fails :) wtf.
    vi idx(m, 0);
    int votes = party[0].size();
    for (int p = 1; p < m; p++) {
      int k = party[p].size();
      while (k - idx[p] >= tot) {
        cost += party[p][idx[p]++];
    ...
    pq<pi> pq;
    for (int p = 1; p < m; p++) {
      if (idx[p] < party[p].size())
        pq.push({-party[p][idx[p]], p});
    }
    while (pq.size() && votes < tot) {
