...
template <integral T, predicate<T> Pred>
T maxst(T l, T r, Pred pred) { return minst(l, r + 1, [pred](T x) { return !pred(x); }) - 1; }
...
void solve() {
  int n = ri();
  int segs = *ranges::min_element(views::iota(1, n + 1), ranges::less{}, [n](int segs) {
    return segs + n / segs + !!(n % segs);
  });
  // cout << segs << endl;
  int block = n;
  vi a;
  for (int i = 0; i < segs; i++) {
    int jn = n / segs + (i < n % segs);
    block -= jn;
    for (int j = 0; j < jn; j++) {
      a.pb(block + j + 1);
    }
