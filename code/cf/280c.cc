...
// editorial: for the deletion of each node, either it got deleted or
//            one of its ancestor got deleted. any node being selected
//            is equiprobable so the contribution of node u is dep[u] + 1
//
// um_nik: instead of doing the random choices one by one (which is inconvenient
//         because the number of remaining options is changing all the time, it
//         depends on the previous choices in some difficult way, which means
//         that probabilities for the next choice are hard to calculate), let's
//         fix a random permutation of all elements beforehand, and then to simulate
//         the process we will go through the permutation and try to apply the removal
//         to the element in the permutation. if it's already removed (by some
//         previous operation) - do nothing. otherwise do the removal
