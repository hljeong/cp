...
struct Graph : vec<vi> {
  ...
  Graph(int n, int m, bool, int off = 1) : Graph(n) {
    for (int i = 0; i < m; i++) {
      def(int, u, v);
      cto(u - off, v - off);
    }
  ...
  void cto(int from, int to, ll weight = 1) { (*this)[from].pb(to); w[from][to] += weight; }
...
void solve() {
  def(int, n);
  Graph g(n);
  for (int u = 0; u < n; u++) g.cto(u, ri() - 1);
  auto g_t = g.transpose();
  auto comps = scc(g);
  vi ret(n);
  for (auto &comp : comps) if (comp.size() > 1 || g[comp[0]][0] == comp[0])
    for (int u : comp)
      ret[u] = u + 1;

  function<void(int, int, int)> dfs = [&](int u, int p, int r) {
    ret[u] = r + 1;
    for (int v : g_t[u]) if (v != p && !ret[v])
      dfs(v, u, r);
  };
