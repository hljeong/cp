...
pair<vc<mint>, vc<mint>> init_fact(int n) {
  ckmax(n, 1);
  vc<mint> fact(n + 1), ifact(n + 1);
  fact[0] = fact[1] = ifact[0] = ifact[1] = 1;
  for (int i = 2; i <= n; ++i) fact[i] = i * fact[i - 1];
  ifact[n] = 1 / fact[n];
  for (int i = n; i > 2; --i) ifact[i - 1] = i * ifact[i];
  return {fact, ifact};
}

int main() {
  cin.tie(0) -> sync_with_stdio(0);
  auto [fact, ifact] = init_fact(4050);
  auto dv = sieve(1000005);
