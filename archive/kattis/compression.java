...
class compression {
    ...
    public static void main(String[] args) throws IOException {
        ...
        int lgx0 = lg(x[0]);
        for (int i = lgx0 + (x[0] != 0 ? 1 : 0); i <= 30; ++i) {
            dp[0][0][i] = min(dp[0][0][i], c + i);
            best[0] = min(best[0], dp[0][0][i]);
        }
        for (int i = lgx0; i <= 30; ++i) {
            dp[0][1][i] = min(dp[0][1][i], c + i);
            best[0] = min(best[0], dp[0][1][i]);
        }
        for (int i = 1; i < n; ++i) {
            int lgxi = lg(x[i]);
            int lgd = lg(x[i] - x[i - 1]) + (x[i] != x[i - 1] ? 1 : 0);
            for (int j = lgd; j <= 30; ++j) {
                dp[i][0][j] = minof(dp[i][0][j], best[i - 1] + c + j, dp[i - 1][0][j] + j);
                best[i] = min(best[i], dp[i][0][j]);
            }
            for (int j = lgxi; j <= 30; ++j) {
                dp[i][1][j] = minof(dp[i][1][j], best[i - 1] + c + j, dp[i - 1][1][j] + j);
                best[i] = min(best[i], dp[i][1][j]);
            }
        }
        prln(best[n - 1]);
        close();
